	...
	for {
		...
		default:
			for {
				...
				if err != nil {
					err = d.sendResponse(decodedMessage.Target, []byte(shared.InvalidRequestResponsePayload))
					if err != nil {
				...
				err = d.sendResponse(decodedMessage.Target, []byte(shared.DefaultSuccessResponsePayload))
				if err != nil {
