...
func (u UnregisterMessageHandler) ProcessMessage(message transport.Message) error {
	componentID, err := u.GetComponentByID(*message.SenderID)

	if err != nil {
		u.Logger.Error(fmt.Sprintf("Failed to find component with ID '%s': %s", *message.SenderID, err.Error()), false, shared.MainComponentName)
	...
	u.ComponentRegistrar.RemoveRegisteredComponent(*message.SenderID)
	return nil
}
